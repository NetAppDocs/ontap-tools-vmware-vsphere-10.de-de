...
[role="lead"]
Wenn Sie einen Datastore auf der Host-Cluster-Ebene erstellen, wird der Datastore auf allen Ziel-Hosts erstellt und gemountet. Diese Option wird nur angezeigt, wenn Sie über die erforderliche Privileges verfügen.
