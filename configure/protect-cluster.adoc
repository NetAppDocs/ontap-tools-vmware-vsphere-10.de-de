...
[role="lead"]
ONTAP tools for VMware vSphere verwaltet den Schutz von Host-Clustern. Alle Datenspeicher, die zur ausgewählten SVM gehören und auf einem oder mehreren Hosts des Clusters eingebunden sind, werden unter einem Host-Cluster geschützt.
...
+
Wenn Sie die SnapMirror Beziehung als AutomatedFailOverDuplex Typ-Richtlinie hinzufügen, müssen Sie die Ziel-Speicher-VM als Speicher-Backend zu demselben vCenter hinzufügen, auf dem ONTAP tools for VMware vSphere bereitgestellt ist.

+
Im AutomatedFailOverDuplex-Richtlinientyp gibt es einheitliche und nicht einheitliche Hostkonfigurationen. Wenn Sie die Umschalttaste *Einheitliche Hostkonfiguration* auswählen, wird die Konfiguration der Host-Initiatorgruppe implizit auf dem Zielstandort repliziert. Weitere Informationen finden Sie unter link:../concepts/ontap-tools-concepts-terms.html["Schlüsselkonzepte und -Begriffe"].
