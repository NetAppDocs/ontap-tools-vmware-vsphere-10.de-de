...
. Wählen Sie vVols als *Datenspeichertyp* aus.
...
. Wählen Sie im Bereich *Speicher* eine Speicher-VM aus. Geben Sie im Abschnitt *Erweiterte Optionen* bei Bedarf den *Namen der benutzerdefinierten Initiatorgruppe* an. Sie können eine vorhandene Initiatorgruppe für den Datenspeicher auswählen oder eine neue Initiatorgruppe mit einem benutzerdefinierten Namen erstellen.
+
...
NOTE: Im ASA r2 Speichertyp gilt die Erstellung oder Auswahl von Volumes nicht für die Erstellung von Speichereinheiten (LUN/Namespace). Daher werden diese Optionen nicht angezeigt.
