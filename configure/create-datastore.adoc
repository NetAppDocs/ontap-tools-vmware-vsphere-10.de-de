...
. Unter Advanced options können Sie *Custom Export Policy* für das NFS-Protokoll oder einen benutzerdefinierten Initiatorgruppennamen für die iSCSI- und FC-Protokolle auswählen, dies wird jedoch nicht empfohlen. Wenn Sie diese Option verwenden, stellen Sie sicher, dass Sie die Erkennung in vCenter für alle Objekte ausführen.
+
...
. Sie können unter „Erweiterte Optionen“ eine benutzerdefinierte Exportrichtlinie auswählen. Dies wird jedoch nicht empfohlen. Falls verwendet, stellen Sie sicher, dass Sie die Erkennung in vCenter für alle Objekte ausführen.
+
