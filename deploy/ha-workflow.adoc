...
[role="quick-margin-para"]
Um vVols Datenspeicher zu konfigurieren, müssen Sie den VASA Provider-Dienst aktivieren. Registrieren Sie den VASA Provider bei vCenter und stellen Sie sicher, dass Ihre Speicherrichtlinien die HA-Anforderungen erfüllen, einschließlich korrekter Netzwerk- und Speicherkonfigurationen.
