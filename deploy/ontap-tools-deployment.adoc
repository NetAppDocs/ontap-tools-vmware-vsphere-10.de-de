...
. Wählen Sie die OVA-Datei aus der URL aus, oder navigieren Sie zu ihrem Speicherort und wählen Sie dann *Weiter*.
...
. Geben Sie im Feld *Namen und Ordner auswählen* den Namen der virtuellen Maschine ein und wählen Sie deren Speicherort. Wenn Sie das Kontrollkästchen *Hardware dieser virtuellen Maschine anpassen* aktivieren, wird ein zusätzlicher Schritt namens *Hardware anpassen* aktiviert, bevor Sie das Fenster *Ready to Complete* erreichen.
...
. Aktivieren Sie im Fenster *Hardware anpassen* CPU Hot add und Memory Hot Plugin.
