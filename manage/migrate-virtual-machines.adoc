...
Snapshots virtueller Maschinen ohne Speicher werden unterstützt. Wenn eine virtuelle Maschine einen Snapshot mit Speicher hat, kann die virtuelle Maschine nicht geschützt werden.

Sie können ungeschützte virtuelle Maschinen, die Speicherabbilder enthalten, nicht schützen. Löschen Sie die Speicherabbilder, bevor Sie den Schutz für die virtuelle Maschine aktivieren.

Für eine Windows-VM mit ASA r2-Speichertyp sind virtuelle Maschinen-Snapshots schreibgeschützt. Wenn Sie die VM einschalten, erstellt der VASA Provider eine LUN aus dem schreibgeschützten Snapshot und aktiviert IOPS. Wenn Sie die VM ausschalten, löscht der VASA Provider die LUN und deaktiviert IOPS.
