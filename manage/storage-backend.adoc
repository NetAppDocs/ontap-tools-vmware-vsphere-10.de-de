...
Sie können globale ONTAP-Cluster mit ONTAP tools Manager entfernen.
...
* Privileges erlaubt und eingeschränkt für das Storage-Backend. Für das untergeordnete SVM sind die Informationen dieselben wie für das übergeordnete Storage-Backend. ONTAP tools zeigt Privileges nur bei clusterbasierten Storage-Backends an. Wenn Sie SVM als Storage-Backend hinzufügen, werden keine Privileges-Informationen angezeigt.
