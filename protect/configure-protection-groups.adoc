...
NOTE: Der Storage Replication Adapter (SRA) unterstützt Fan-Out mit einer synchronen Beziehung vom Typ Automated Failover Duplex und einer asynchronen SnapMirror-Beziehung in einer Konsistenzgruppe. Fan-Out mit zwei asynchronen SnapMirror-Beziehungen in Konsistenzgruppen oder Fan-Out-SnapMirror-Beziehungen in Volumes wird jedoch nicht unterstützt. Vault-Typ-SnapMirror-Beziehungen sind von diesen Fan-Out-Beschränkungen ausgenommen. Die Unterstützung von Konsistenzgruppen im SRA ist auf SAN-Protokolle (iSCSI und Fibre Channel) auf Unified- und ASA r2-Speichersystemen beschränkt. Konsistenzgruppen werden für NFS nicht unterstützt. Wenn NFS-Datenspeichervolumes einer Konsistenzgruppe zugeordnet werden, schlägt die Geräteerkennung fehl.
