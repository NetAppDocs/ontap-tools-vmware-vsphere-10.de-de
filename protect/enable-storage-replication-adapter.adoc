...
.Bevor Sie beginnen
* Sie sollten Ihre vCenter Server-Instanz eingerichtet und die ESXi-Hosts konfiguriert haben.
...
.Schritte
. Melden Sie sich an der Managementoberfläche der VMware Live Site Recovery Appliance über die URL  `\https://<srm_ip>:5480` an und gehen Sie dann zu Storage Replication Adapters in der Managementoberfläche der VMware Live Site Recovery Appliance.
